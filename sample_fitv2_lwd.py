    ...
    with torch.no_grad():
        ...
        with accelerator.autocast():
            if isinstance(ema_model, torch.nn.parallel.DistributedDataParallel):
                ...
                #output_test = ema_model.forward_cfg(noise_test, t_test, 1.5, **model_kwargs_test, number_of_step_perflow=250)
        ...
        if accelerator.is_main_process:
            for i, img_tensor in enumerate(samples):
                img = Image.fromarray(img_tensor.cpu().numpy())
                img.save(os.path.join(f'{workdirnow}', f"images/fitv2_sample_test-{i}-NFE2.jpg"))
        ...
        if accelerator.is_main_process:
            for i, img_tensor in enumerate(samples):
                img = Image.fromarray(img_tensor.cpu().numpy())
                img.save(os.path.join(f'{workdirnow}', f"images/fitv2_sample_test-{i}-NFE4.jpg"))
        ...
        if accelerator.is_main_process:
            for i, img_tensor in enumerate(samples):
                img = Image.fromarray(img_tensor.cpu().numpy())
                img.save(os.path.join(f'{workdirnow}', f"images/fitv2_sample_test-{i}-NFE20.jpg"))
