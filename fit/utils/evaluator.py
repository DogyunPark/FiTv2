...
class FIDStatistics:
    ...
    def frechet_distance(self, other, eps=1e-6):
        ...
        #return diff.dot(diff), np.trace(sigma1) + np.trace(sigma2) - 2 * tr_covmean, diff.dot(diff) + np.trace(sigma1) + np.trace(sigma2) - 2 * tr_covmean
...
@contextmanager
def _open_npy_file_(path: str, arr_name: str):
    return np.load(path)
    #with open(path, "rb") as f:
        #with zipfile.ZipFile(f, "r") as zip_f:
        #    if f"{arr_name}.npy" not in zip_f.namelist():
