...
class FiTLwD(nn.Module):
    ...
    def forward(self, x, t, cfg, y, grid, mask, size=None, number_of_layers_for_perflow=None):
        """
        Forward pass of FiT.
        ...
        if not self.use_checkpoint:
            for i, block in enumerate(self.blocks):
                ...
                if i_drop == 0:
                    t = self.sigmas[i_mod].expand(x.shape[0]).to(x.device)
                    ...
                    if self.fourier_basis is not None:
                        t_next = self.sigmas[i_mod+1].expand(x.shape[0]).to(x.device)
                ...
                if i_finish == 0:
                    ...
                    x = (self.sigmas[i_mod+1] - self.sigmas[i_mod]) * x + residual
        else:
            for i, block in enumerate(self.blocks):
                i_mod = i // number_of_layers_for_perflow
                i_drop = i % number_of_layers_for_perflow
