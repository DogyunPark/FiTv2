...
from fit.model.modules_lwd_bk import (
    ...
    FiTBlock, FinalLayer, RepresentationBlock, TimestepDependentCoefficient
)
...
class FiTLwD_sharedenc_sepdec(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.t_embedder_rep = TimestepEmbedder(hidden_size)
        ...
        if finetune_representation:
            self.mid_block = nn.ModuleList([FiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, swiglu=use_swiglu, swiglu_large=use_swiglu_large,
                rel_pos_embed=rel_pos_embed, add_rel_pe_to_v=add_rel_pe_to_v, norm_layer=norm_type, 
                q_norm=q_norm, k_norm=k_norm, qk_norm_weight=qk_norm_weight, qkv_bias=qkv_bias, ffn_bias=ffn_bias, 
                adaln_bias=adaln_bias, adaln_type=adaln_type, adaln_lora_dim=adaln_lora_dim, concat_adaln=concat_adaln) for _ in range(number_of_perflow)])
            self.coefficient_layers = nn.ModuleList([TimestepDependentCoefficient(hidden_size) for _ in range(number_of_perflow)])
            #self.finetune(type=finetune, unfreeze=('mid_block', 'coefficient_layers'))
    ...
    def initialize_weights(self, pretrain_ckpt=None, ignore=None):
        ...
        nn.init.normal_(self.t_embedder_rep.mlp[0].weight, std=0.02)
        nn.init.normal_(self.t_embedder_rep.mlp[2].weight, std=0.02)
    ...
    def forward_wo_cfg(self, x, t, cfg, y, number_of_step_perflow=1, noise=None, representation_noise=None):
        ...
        for i in range(len(self.blocks) // self.number_of_layers_for_perflow):
            ...
            sigma_list_ori = torch.linspace(sigma_current, sigma_next, number_of_step_perflow+1)
            sigma_list = torch.linspace(0, 1, number_of_step_perflow+1)

            for step in range(number_of_step_perflow):
                t = sigma_list_ori[step].expand(x.shape[0]).to(x.device)
                t = torch.clamp(self.time_shifting * t / (1  + (self.time_shifting - 1) * t), max=1.0)        
                t = t.float().to(x.dtype)
                t_rep = self.t_embedder_rep(t)
                c = t_rep + y_embed
    ...
    def forward_run_layer(self, x, t, cfg, y, target_layer_start=None, target_layer_end=None, 
        ...
        #     dummy_sum = dummy_sum + 0.0 * param.sum()
        # x = x + 0.0 * dummy_sum
        ...
        t_next = self.t_embedder[target_layer_start//self.number_of_layers_for_perflow](t_next)
        c_repre = t_next.unsqueeze(1) + representation_noise
        #c_repre = torch.cat([c.unsqueeze(1).repeat(1, representation_noise.shape[1], 1), representation_noise], dim=-1)
    ...
    def forward_run_layer_finetune(self, x, t, cfg, y, target_layer_start=None, target_layer_end=None, 
        ...
        coefficient = self.coefficient_layers[target_layer_start//self.number_of_layers_for_perflow](t_next)
        ...
        return x_pred, x_target, representation_linear, None
    ...
    def forward_cfg(self, x, t, cfg, y, number_of_step_perflow=1, noise=None, representation_noise=None):
        ...
        for i in range(len(self.blocks) // self.number_of_layers_for_perflow):
            ...
            for step in range(number_of_step_perflow):
                t = sigma_list_ori[step].expand(x.shape[0]*2).to(x.device)
                t = torch.clamp(self.time_shifting * t / (1  + (self.time_shifting - 1) * t), max=1.0)        
                t = t.float().to(x.dtype)
        ...
        for i in range(len(self.blocks) // self.number_of_layers_for_perflow):
            ...
            for step in range(number_of_step_perflow):
                ...
                if step != 0:
                    ...
                    c_mid = c.unsqueeze(1) + representation_noise
